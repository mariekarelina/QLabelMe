...
Polyline::CloseMode Polyline::_closeMode = Polyline::CloseMode::CtrlModifier;
...
bool Polyline::insertPointAtIndex(int index, const QPointF& scenePos)
{
    if (!scene())
        return false;
...
bool Polyline::removePointAtIndex(int index)
{
    if (!scene())
        return false;

    // У замкнутой полилинии минимум 3 точки.
    if (_isClosed && _circles.size() <= 3)
        return false;

    // У незамкнутой полилинии минимум 2 точки.
    if (!_isClosed && _circles.size() <= 2)
        return false;
...
bool Polyline::findInsertPointAt(const QPointF& scenePos,
                                 int* pointIndex,
                                 QPointF* point) const
{
    if (_circles.size() < 2)
...
bool Polyline::removeLastPointForce(bool callCallback)
{
    if (!scene())
        return false;

    if (_circles.size() <= 1)
        return false; // оставляем стартовую точку
...
void Polyline::setClosed(bool closed, bool callCallback)
{
    // Полилинию имеет смысл замыкать только при >=3 точках
    if (closed)
    {
    ...
    if (callCallback && _modificationCallback && !_closeCallbackScheduled)
    {
...
void Polyline::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    if (_closeMode == CloseMode::DoubleClick
        && event->button() == Qt::LeftButton)
    {
        int idx = -1;
...
void Polyline::dragCircleRelease(DragCircle* circle)
{
    ...
    if (_isClosed || _closeCallbackScheduled)
        return;
...
void Polyline::replaceScenePoints(const QVector<QPointF>& scenePts, bool closed)
{
